...
ssh "${SSH_OPTS[@]}" "${REMOTE}" "cd '${REMOTE_DIR}' && '${REMOTE_PYTHON}' -m pip show optuna >/dev/null 2>&1 || '${REMOTE_PYTHON}' -m pip install optuna"

ssh "${SSH_OPTS[@]}" "${REMOTE}" "cd '${REMOTE_DIR}' && '${REMOTE_PYTHON}' setup.py -q build_ext --inplace --force"

ssh "${SSH_OPTS[@]}" "${REMOTE}" "cd '${REMOTE_DIR}' && mkdir -p artifacts/weight_tuning"
...
if ssh "${SSH_OPTS[@]}" "${REMOTE}" "command -v tmux >/dev/null 2>&1"; then
  ...
  ssh "${SSH_OPTS[@]}" "${REMOTE}" "cd '${REMOTE_DIR}' && setsid bash -lc \"${REMOTE_COMMAND}\" </dev/null >/dev/null 2>&1 & echo \$! > '${REMOTE_DIR}/artifacts/weight_tuning/search.pid'"

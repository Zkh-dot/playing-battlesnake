...
### Retained completed snapshot timeout labeling

Exact-head rereview found one remaining source transition: when depth N completed and depth N+1 timed out, the retained completed snapshot still published its initial `minimax` source. The contract requires `timeout_best_so_far` whenever timeout termination returns a searched snapshot, whether that snapshot came from a completed iteration or an adopted first partial.

The deterministic seam now takes the termination state. Its RED completed-then-timeout case retained depth 1, move, value, root-comparison reason, and full root tags but incorrectly reported `minimax`. A narrow finalization helper changes only the source to `timeout_best_so_far` when `timed_out` and the snapshot contains a completed depth or a tagged selected partial. A normal completed snapshot remains `minimax`; a partial remains `timeout_best_so_far`; an unsearched fallback remains `allowed_fallback`. The Python smoke again requires `timeout_best_so_far` for `timed_out` with a completed depth, while the deterministic fixed-depth test requires `minimax` for normal completion.

Post-fix validation repeated cleanly: forced native rebuild warning-free; focused issue42/diagnostics/checker `102 passed, 11 subtests`; C server and strict warning build passed; #36/#38/#41 `80 passed, 1 skipped, 14 subtests`; four C position configurations passed; broad non-training/non-server `293 passed, 1 skipped, 52 subtests`; localhost native-server tests `2 passed, 3 subtests`. Comparator, corridor, and budget code are untouched, and `git diff --check` passed.
